...
| **Secrets at runtime** | **chmod-600 file in app-data dir** (interim) → **OS keyring** when codo is signed | ⚠ **Updated 2026-05-08.** Originally OS keyring via the `keyring` crate. Reality during Step 3: every Rust rebuild of an unsigned binary produces a different code signature, so macOS Keychain re-prompts the operator on every read after every rebuild — 20+ prompts an hour during active dev. An unsigned Keychain entry isn't actually more secure than a chmod-600 file (the OS-level "I trust this binary" mechanism keys on the signed bundle identity we don't have). So `~/Library/Application Support/codo/credentials.json` (mode 0600) is the interim store, with one-time auto-migration from any pre-existing Keychain entries on first read. When codo gets an Apple Developer ID + stable signed bundle (§5 #7), put the creds back in Keychain. **Critical** still applies: Turso URL + token NEVER live in a baked-in `.env`. |
